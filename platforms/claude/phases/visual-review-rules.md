...
VISUAL_REVIEW 阶段固定使用**单 Agent 直接调度**（不使用 Agent 工具并行调度），因为：
...
| 方式 | 条件 | 说明 |
|------|------|------|
| **Task 调度**（默认） | 始终优先 | 使用 Agent 工具调用 visual-reviewer Agent |
...
1. 读取你的 Agent 行为规范文件（Read 工具）：{agents/visual-reviewer.md 的绝对路径}
