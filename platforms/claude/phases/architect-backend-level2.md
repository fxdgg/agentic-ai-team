# ARCHITECT_BACKEND — Level 2: Agent 串行流水线

> **加载时机**: 编排器判定使用 Level 2 (Agent 流水线) 模式时按需加载，或从 Level 1 降级时加载。
...
| **断点恢复** | 每个 Agent 调用 完成后中间产物已落盘，降级或中断后可从已完成的 Task 之后继续 |
| **角色合并** | 涉及领域/模块仅 1 个时，全局分析 + 领域文档输出合并为单次 Agent 调用 |
...
[Agent-B ~ Agent-N] 各领域文档输出（每个领域一个 Task，串行）
...
1. 读取你的 Agent 行为规范文件（Read 工具）：{agents/{selected-architect}.md 的绝对路径}
...
1. 确定起始 Task（检查已存在的中间产物）：
 a) architecture.md 不存在 → 从 Agent-A 开始
 b) architecture.md 存在但部分领域 tech-requirements.md 不存在 → 从对应领域 Task 开始
 c) 所有产物已存在 → 直接进入总结确认

2. 执行 Agent-A（如需）：
 a) 使用 Agent 工具，注入 §3 Agent-A Prompt
 b) Agent 返回后，检查全局产物是否成功写入
...
4. 串行执行领域 Task（Agent-B ~ Agent-N）：
 a) 对每个领域，使用 Agent 工具注入 §3 领域 Agent Prompt
 b) 每个 Agent 返回后，检查领域产物是否成功写入
...
| 失败场景 | 处理方式 |
|---------|---------|
| Agent-A 返回空结果 / 无全局产物 | 降级到 Level 3 |
| 领域 Agent 返回空结果 / 无领域产物 | 降级到 Level 3（已有产物可供参考） |
