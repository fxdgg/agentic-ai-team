# ARCHITECT_BACKEND — Level 1: Agent 工具并行调度模式调度规则

> **加载时机**: 编排器判定使用 Level 1 (Agent 工具并行调度) 模式时按需加载。
...
```
发起 Agent 调用组（调度标识: arch-backend-{需求ID}）：

调度任务：基于后端技术需求文档进行架构设计，产出全局架构文档和各领域技术需求文档。
调度采用两步模式：Step 1 全局架构分析（串行），Step 2 领域文档并行输出。
...
### Agent 1: @global-architect（必选，始终创建）

```
生成一个全局架构师Agent（@global-architect），Prompt 如下：
...
1. 读取 Agent 行为规范：{agents/{selected-architect}.md 的绝对路径}
 ...
 — 跳过「阶段三：领域/模块文档输出」（由领域/模块 Agent 完成）
...
⚠️ 重要约束：
- 你只负责全局架构分析和全局产物输出
- 不要输出任何领域/模块的 tech-requirements.md（由领域/模块 Agent 并行完成）
- 你需要在 architecture.md 中列出本次涉及的所有领域/模块及其职责边界，供领域/模块 Agent 引用
...
### Agent 2~N: @domain-architect-{module}（按涉及领域/模块动态生成）

```
生成一个 {领域/模块中文名} 架构Agent（@domain-architect-{module}），Prompt 如下：
 ...
 * 独立业务模块 → 各自作为独立领域
 - 为每个领域发起对应的 @domain-architect-{module} Agent 调用
...
| ✅ 必须做 | ❌ 禁止做 |
|-----------|----------|
| 发起 Agent 调用并分配任务 | 直接执行架构分析/文档输出 |
| 每个 Agent完成后立即更新检查点 | 跳过检查点更新 |
| 监控超时并执行自动降级（§5） | 在超时降级前不发催促消息 |
| 处理澄清问题 | 向领域 Agent传递其他领域 Agent的对话 |

---

## 5. 超时检测与自动降级（Level 1 → Level 2）
...
| 领域 Agent超时且无领域产物 | Level 2（从该领域 Task 开始） |

**降级执行**: 记录原因 → 汇总结果 → 更新 `architectBackendMode = "task-pipeline"` → 跳转到 Level 2 子文件
...
| 消费方 | 消费场景 | 读取字段 |
|--------|---------|---------|
| 编排器（Step 2） | 发起领域架构师 Agent 调用 | `domains[].id`, `domains[].modules` |
...
```
1. 确认 @global-architect 的返回结果和全局产物
2. 逐一确认每个 @domain-architect-{service} 的返回结果和领域产物
3. 执行产物完整性检查：
...
5. 汇总风险项
6. 更新 state.json（architectBackendMode = "parallel-agent"，checkpoint = "domains_completed"）
7. 无需清理 → 进入"总结确认"步骤
```
