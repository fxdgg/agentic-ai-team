...
| 级别 | 模式 | 触发条件 | 子文件 |
|------|------|---------|--------|
| **Level 1** | Agent 工具并行调度 | 涉及领域/模块 ≥2 个，始终优先尝试 | `phases/architect-backend-level1.md` |
| **Level 2** | Agent 串行流水线 | 领域/模块仅 1 个，或 Level 1 创建失败/超时 | `phases/architect-backend-level2.md` |
...
2. 如果 architectBackendMode 已有值且非空：
 a) "parallel-agent" → 使用 Read 工具加载 "phases/architect-backend-level1.md"，进入 Level 1 断点恢复
 b) "task-pipeline" → 使用 Read 工具加载 "phases/architect-backend-level2.md"，进入 Level 2 断点恢复
 c) "fallback" 或 "task" → 使用 Read 工具加载 "phases/architect-backend-level3.md"，进入 Level 3 恢复
3. 如果 architectBackendMode 为空或字段不存在：
 → 尝试 Level 1:
 a) 使用 Read 工具加载 "phases/architect-backend-level1.md"
 b) 按 Level 1 规则执行
 c) 失败或超时 → 使用 Read 工具加载 "phases/architect-backend-level2.md"，降级到 Level 2
 d) Level 2 也失败 → 使用 Read 工具加载 "phases/architect-backend-level3.md"，降级到 Level 3
```
...
> **检查点 1 是"上下文防火墙"**: @global-architect 的搜索上下文（~100KB）被压缩为 3 个结构化文档（~20KB），领域 Agent的上下文因此保持干净。
...
| 领域 Agent陆续完成时 | `domains_pending` | 每完成一个即更新 |
| 所有领域 Agent完成后 | `domains_completed` | 阶段完成 |

### 2.2 检查点更新流程

```
每个产物落盘后：
 1. 编排器 Read 工具 确认产物文件存在且非空
...
2. 根据 step 值确定恢复点：
 ...
 - "domains_pending" → 仅创建 pendingDomains 中未完成的领域 Agent
...
| 场景 | 处理方式 |
|------|---------|
| @global-architect 失败 | 发送修复指令，或创建替代 Agent 调用 |
| @domain-architect-{service} 失败 | 全局产物已存在，可重新发起 Agent 调用重试 |
| Agent 调用失败/返回空结果 | Level 1 子文件 §5 的超时检测流程 |
...
```
验证流程：
1. 使用 Read 工具读取 architecture.md → 确认非空
2. 使用 Read 工具读取 dependency-graph.md → 确认包含 Mermaid 图
3. 使用 Read 工具读取 priority-list.md → 确认包含优先级表
...
**Step 2: 执行** —
- **Level 1**: 发起 Agent 调用 → S1 全局分析 → 检查点 → Step 1.5 领域确认 → S2 并行领域文档 → 逐个检查点 → 清理
- **Level 2**: Agent-A 全局分析 → [领域确认] → Agent-B~N 串行领域文档 → 检查产物
...
| 子文件 | 行数 | 加载条件 | 包含内容 |
|--------|------|---------|---------|
| `phases/architect-backend-level1.md` | ~340 | 走 Level 1 路径时 | Agent 工具并行调度 完整规则：Prompt 模板、领域治理、确认关卡、超时降级、domain-registry Schema |
| `phases/architect-backend-level2.md` | ~180 | 走 Level 2 路径时 | Agent 流水线定义 + Prompt 模板 + 执行流程 + 失败处理 |
