# ANALYSE_TECH 阶段 Agent Teams 调度规则（按需加载）
...
ANALYSE_TECH 阶段支持两种调度模式，编排器**优先使用 Agent Teams 模式**：

| 模式 | 触发条件 | 说明 |
|------|---------|------|
| **Agent Teams 模式**（默认） | 始终优先使用 | 使用 Agent Teams 四成员串行协作，独立上下文窗口隔离搜索结果膨胀 |
| **Task 工具模式**（降级） | Agent Teams 创建失败 | 回退到传统 Task 工具调用单体 `fullstack-analyst.md` |

> **设计意图**: Agent Teams 的核心价值在 ANALYSE_TECH 阶段是**上下文隔离**而非并行执行。通过将代码搜索（上下文消耗最大的工作）隔离在独立窗口中，避免搜索结果膨胀导致上下文压缩，从而保障复杂规则（复用探索、证据链、接口契约、一致性校验）的执行精度。
...
## 2. Agent Teams 模式调度规则

### 代码画像注入协议（仅当 `knowledgeContext.profilePath` 存在）

> **加载条件**: 编排器在创建 Agent Teams 前，先读取 `state.json` 检查 `knowledgeContext.profilePath` 字段。
...
使用方式:
1. 在 Step 1 初始化时，先 read_file 读取画像文件
...
## 4. state.json 中的 Agent Teams 记录
...
- 调度模式（Agent Teams / Task 工具降级）
- Agent Teams 模式下：团队名称、四个成员的职责、依赖关系
- 启用的平台列表
- 产品需求文档概要

**Step 2: 执行** —
- **Agent Teams 模式**: 创建分析团队 → 分配任务（T1→T2→T3→T4 串行） → 监控完成 → 汇总结果 → 清理团队
