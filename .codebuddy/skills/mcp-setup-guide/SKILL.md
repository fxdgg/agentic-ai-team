...
本技能引导用户完成 CodeBuddy MCP 服务的配置，包括全局配置文件生成、Token 申请和连通性验证。
...
```
配置文件路径: ~/.codebuddy/mcp.json（即 $HOME/.codebuddy/mcp.json）

检查项:
  1. 文件是否存在
  2. 文件是否包含有效的 JSON 格式
  3. mcpServers 中是否包含目标服务（tapd_mcp_http / iWiki / FramelinkFigmaMCP）
  4. 对应服务的 headers 或 args 中是否包含有效 Token（非占位符）
```
...
在全局 `~/.codebuddy/` 目录下创建或更新 `mcp.json`，使用占位符替代真实 Token：
...
```
✅ 全局 MCP 配置文件已生成: ~/.codebuddy/mcp.json
...
使用 `ask_followup_question` 工具引导用户申请 iWiki 所需的 TAI PAT Token：
...
```
将 ~/.codebuddy/mcp.json 中 iWiki 服务的 YOUR_TAI_PAT_TOKEN 替换为用户提供的实际 Token
  - iWiki 的 Authorization 值: "Bearer {token}"
```

#### 3b. TAPD Token（TAPD 个人访问令牌）

使用 `ask_followup_question` 工具引导用户申请 TAPD 所需的个人访问令牌：
...
```
将 ~/.codebuddy/mcp.json 中 TAPD 服务的 YOUR_TAPD_ACCESS_TOKEN 替换为用户提供的实际 Token
  - TAPD 的 X-Tapd-Access-Token 值: "Bearer {token}"
```

#### 3c. Figma Token（Figma Personal Access Token）

使用 `ask_followup_question` 工具引导用户申请 Figma 所需的个人访问令牌：
...
5️⃣ 输入 Token 描述（如 "CodeBuddy MCP"），设置权限和过期时间后确认生成
...
```
将 ~/.codebuddy/mcp.json 中 FramelinkFigmaMCP 服务 args 中的 YOUR_FIGMA_PERSONAL_ACCESS_TOKEN 替换为用户提供的实际 Token
...
2. **替换后必须验证**：所有替换完成后，必须重新 read_file 读取 ~/.codebuddy/mcp.json，
...
1. 所有 MCP 配置直接在**全局配置文件**（`~/.codebuddy/mcp.json`）中维护，不在项目内创建 `mcp.json`
...
7. FramelinkFigmaMCP 是本地 stdio 类型 MCP（通过 `npx figma-developer-mcp` 启动），不走 HTTP 协议，无需 curl 验证；重新加载 MCP 配置后 CodeBuddy 会自动启动并连接
