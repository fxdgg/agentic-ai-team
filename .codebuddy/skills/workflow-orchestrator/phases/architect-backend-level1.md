# ARCHITECT_BACKEND — Level 1: Agent Teams 模式调度规则

> **加载时机**: 编排器判定使用 Level 1 (Agent Teams) 模式时按需加载。
...
| 领域成员超时且无领域产物 | Level 2（从该领域 Task 开始） |

**降级执行**: 记录原因 → 清理团队 → 更新 `architectBackendMode = "task-pipeline"` → 跳转到 Level 2 子文件
