...
IMPLEMENT 阶段**锁定使用 Agent Teams 模式**，即使仅 1 个 Agent 也使用 Agent Teams 以保持上下文纯粹：

| 模式 | 触发条件 | 说明 |
|------|---------|------|
| **Agent Teams 模式**（锁定） | 始终使用 | 使用 Agent Teams 独立上下文窗口，即使仅 1 个 Agent 也创建单成员团队 |
| **用户决策**（创建失败时） | Agent Teams 创建失败 | 报错给用户决策，不自动降级到 Task 模式 |

> **设计意图**：借鉴 agentic-mall 的生产经验——IMPLEMENT 阶段即使仅 1 个 Agent，也使用 Agent Teams 以保持编排器上下文纯粹。开发 Agent 的代码搜索和写入操作会产生大量上下文，隔离在独立窗口中可避免编排器上下文膨胀，确保后续 BUILD_VERIFY 阶段的状态读取精度。创建失败时报错由用户决策，而非自动降级到上下文共享的 Task 模式。
...
- Agent Teams 的任务依赖机制会自动阻塞下游任务，P(N-1) 完成后自动解除 P(N) 的阻塞
- 同优先级的任务可**真并行执行**
...
当 Agent Teams 创建失败时，编排器**不自动降级**，而是：

1. 向用户展示错误信息
2. 提供选项：
   - **重试**：重新尝试创建 Agent Teams
   - **手动处理**：用户自行在 IDE 中执行开发任务
   - **强制降级**：用户明确要求后，使用编排器直接执行模式（编排器加载 Agent 规范自行执行，上下文压力大，产出精度可能降低）

> **CRITICAL**：编排器**严禁自行决定降级**。即使检测到仅 1 个 Agent 需调度，也必须尝试创建 Agent Teams。
...
### 5.3 Agent Teams 修复模式的特殊规则

在编译修复模式下使用 Agent Teams 时：
...
2. 加载 figma-d2c Skill：
   - 使用 use_skill('figma-d2c') 加载 D2C Skill
   - 传入嵌入模式上下文：
     {
       mode: "d2c-embedded",
   ...
   >   - 若当前在 Agent Teams 环境中 → Multi-Agent 模式（Coordinator 调度 7 个 Agent）
...
### 6.2 D2C 嵌入模式与 Agent Teams 的关系

| 场景 | 调度策略 |
|------|---------|
| D2C 嵌入 + 后端多领域 | Agent Teams 模式：D2C 作为 `@d2c-frontend` 成员加入团队，与后端领域 Agent 并行 |
| D2C 嵌入 + 仅前端 | Task 工具模式（降级）：仅 1 个 Agent（D2C），直接用 Task 调用 |
| D2C 嵌入 + 后端单领域 | Agent Teams 模式：2 个 Agent（@d2c-frontend + @{domain}-dev） |

**D2C 作为 Agent Teams 成员时的 Prompt 模板**：
...
Agent Teams 模式下的断点恢复有特殊限制（Agent Teams 不支持 /resume 恢复成员）：
...
| import 一致性 | `grep -r "import {类名}" {目录} --include="*.java"` | Agent 用 codebase_search 搜索 | ~5KB → ~0.3KB |
| 接口签名校验 | `grep -n "public.*{方法名}" {文件路径}` | Agent read_file 整个文件再解析 | ~20KB → ~0.2KB |
