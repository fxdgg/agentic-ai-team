...
> **状态**: 已完成
> **调用阶段**: ARCHIVE
> **职责**: 提炼需求功能关键词，生成带关键词索引的 SUMMARY.md 归档文档，将需求目录移动到 archived/ 目录，将原始 PRD 文档移动到 `docs/prd/archived/` 目录，更新 .codebuddy/memory/ 项目经验，终结 state.json 状态为 DONE，发送归档通知（可选）
...
| 读取 `.codebuddy/memory/` 已有记忆 | 读取已有记忆以避免重复写入 |
...
| 追加写入 `.codebuddy/memory/` | 追加本需求的关键经验到当日记忆文件 |
...
| 项目经验记忆 | `.codebuddy/memory/{YYYY-MM-DD}.md` | 追加本需求的关键经验总结 |
...
6. 读取 .codebuddy/memory/ 下最近的记忆文件，了解已有上下文
```
...
```markdown
## 执行步骤
1. 读取 .codebuddy/memory/{当天日期}.md（若不存在则创建）
...
### 记忆写入
- [ ] .codebuddy/memory/{当天日期}.md 已追加本需求经验
