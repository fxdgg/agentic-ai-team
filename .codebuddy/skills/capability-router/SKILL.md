...
能力路由器维护一份动态的 Skill 注册表，从 `.codebuddy/skills/` 目录扫描：
...
路由器启动时自动扫描 `.codebuddy/skills/*/SKILL.md` 的 frontmatter：

```
扫描规则:
1. 遍历 .codebuddy/skills/ 下的一级子目录
...
当 CodeBuddy 接收到用户请求时，capability-router 在后台进行意图匹配：
...
```
.codebuddy/skills/{new-skill-name}/
