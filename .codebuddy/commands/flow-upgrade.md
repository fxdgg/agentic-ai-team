---
name: flow-upgrade
description: 工作流版本更新。从 ur-ai-team 远程仓库拉取最新版本，对比差异后选择性更新本地 .codebuddy/ 目录。
---
...
本指令用于将当前项目中的 `.codebuddy/` 工作流引擎更新到最新版本。通过版本对比展示变更内容，支持全量更新或选择性更新。
...
4. 如果克隆失败:
   → 提示用户检查网络和 SSH 配置
   → 提供手动更新方案: "您也可以手动 git clone 后执行 cp -r .codebuddy/ 覆盖"
   → 终止流程
```
...
```
1. 遍历 .ai-team-upgrade/.codebuddy/ 下的所有文件
2. 按模块分组对比:
...
1. 对每个选中的模块:
   a) 复制远程文件到本地 .codebuddy/ 对应路径
   b) 新增文件: 直接复制
   c) 变更文件: 覆盖（本地旧版本不备份，可通过 git 恢复）
   d) 本地自定义文件: 保留不动

2. 更新 .codebuddy/.ai-team-version:
   {
...
1. 检查关键文件完整性:
   ...
   - .codebuddy/skills/knowledge-evolution/SKILL.md 是否存在
...
1. **memory/ 目录保护**：更新永远不会覆盖 `.codebuddy/memory/` 目录，组织记忆是项目特有的
...
5. **可回滚**：如果项目本身在 Git 管理下，更新前的文件可通过 `git checkout -- .codebuddy/` 回滚
