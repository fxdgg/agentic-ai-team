...
2. **展示修改预览**：
   ...
   **目标文件**：.codebuddy/skills/workflow-orchestrator/agents/web-developer.md
   **同步文件**：.claude/skills/workflow-orchestrator/agents/web-developer.md
...
3. **执行修改**：Owner 确认后，使用 `replace_in_file` 工具执行修改。**必须同时修改 `.codebuddy/` 和 `.claude/` 下的对应文件**，保持双平台同步。
...
### 被修改的流水线文件
- `.codebuddy/skills/workflow-orchestrator/agents/web-developer.md`
- `.claude/skills/workflow-orchestrator/agents/web-developer.md`（同步）
  - [+] 新增路由注册检查规则
...
2. **发送方式**（路径：`.codebuddy/skills/send-flow-message/send.py`）：

   ```bash
   echo '消息内容' | python3 .codebuddy/skills/send-flow-message/send.py --tag evolve-apply
   ```
...
2. **双平台同步修改**：修改范围覆盖 `.codebuddy/skills/workflow-orchestrator/` 和 `.claude/skills/workflow-orchestrator/` 下的 Agent 和 Rule 文件，每次落地必须同时更新两个平台目录
