...
1. **阅读 Agent 注册表**：
   - 读取 `.codebuddy/skills/workflow-orchestrator/SKILL.md` 中的 §3 子 Agent 注册表
   - 理解每个 Agent 的职责边界
...
3. **生成改进建议**（针对 🟠 或 🔴 风险阶段）：
   ...
   - **压缩事件频繁**（≥2 次）→ 建议增加 Agent Teams 并行度分散压力、中间产物进一步压缩
...
1. **调用 send.py 脚本**（路径：`.codebuddy/skills/send-flow-message/send.py`），发送审核提醒。
...
3. **发送方式**：
   ...
   ```bash
   echo '消息内容' | python3 .codebuddy/skills/send-flow-message/send.py --tag evolve
   ```
