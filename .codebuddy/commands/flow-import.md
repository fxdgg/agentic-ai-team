...
使用 `ask_followup_question` 工具引导用户提供信息（**多选**）：
...
对每个 git 仓库地址:
  ...
  2. 确定克隆目标目录（默认: 当前工作区根目录下）
     - 克隆路径: {workspace}/{repo-name}
       如工作区为 /Users/xxx/CodeBuddy/ai-team，仓库名为 general-chatbot
       → 克隆到 /Users/xxx/CodeBuddy/ai-team/general-chatbot
...
判断结果:
  - 文件不存在 / 缺少 tapd_mcp_http / Token 为占位符:
    → 调用 use_skill('mcp-setup-guide') 引导用户完成 MCP 配置
    → 配置完成后继续 TAPD 拉取流程
  - 配置正常:
    → 继续执行 2b-1 的链接解析流程
```
...
> 当 CodeBuddy 已加载 MCP 配置后，可直接使用 `mcp__tapd_mcp_http__lookup_tapd_tool`、`mcp__tapd_mcp_http__proxy_execute_tool` 等 MCP 原生工具调用，无需手动构造 HTTP 请求。
...
```
前置: 调用 use_skill('tapd-toolkit') 加载 TAPD 扩展技能（图片/附件操作需要）
...
对每个 TAPD 链接:
  ...
  2. TAPD 能力不可用处理（统一降级策略）:
     - MCP 配置缺失（mcp.json 不存在或缺少 tapd_mcp_http）:
       → 调用 use_skill('mcp-setup-guide') 引导用户完成配置
       → 配置完成后重试
...
按文件扩展名自动选择解析方式:
  - .md / .txt        → 直接 read_file
  - .pdf              → 调用 pdf skill（use_skill('pdf')）解析提取文本
  - .docx             → 调用 docx skill（use_skill('docx')）解析提取文本
  - .pptx             → 调用 pptx skill（use_skill('pptx')）解析提取文本
  - 其他扩展名         → 尝试 read_file，失败则跳过并提示用户
```
...
调用 `use_skill('workflow-orchestrator')` 并传入特殊模式标识：
