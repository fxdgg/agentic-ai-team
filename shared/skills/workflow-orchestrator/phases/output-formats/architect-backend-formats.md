# ARCHITECT_BACKEND 阶段 Agent Teams 输出格式
...
## 16. ARCHITECT_BACKEND 阶段 Agent Teams 预览格式

```
** 📋 阶段预览: ARCHITECT_BACKEND（Agent Teams 模式） **

📌 需求: [需求名称]
🔄 当前阶段: ARCHITECT_BACKEND (5/13)
🚀 调度模式: Agent Teams 两步模式（全局架构 → 领域确认 → 领域文档并行输出）
...
## 17. ARCHITECT_BACKEND 阶段 Agent Teams 进度监控格式

```
** 🔄 ARCHITECT_BACKEND 进度: Agent Teams **
...
## 18. ARCHITECT_BACKEND 阶段 Agent Teams 总结格式

```
** ✅ 阶段完成: ARCHITECT_BACKEND（Agent Teams 模式） **

🏷️ 团队: arch-backend-[需求ID]
⏱️ 总耗时: [时长]
🚀 调度模式: Agent Teams 两步模式（1 个全局成员 + [N] 个领域成员）
