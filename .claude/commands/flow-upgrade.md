...
```
1. 读取 .claude/.ai-team-version 文件（如存在）:
   {
     "commitHash": "abc1234",
...
4. 如果克隆失败:
   → 提示用户检查网络和 SSH 配置
   → 提供手动更新方案: "您也可以手动 git clone 后执行 cp -r .claude/ 覆盖"
   → 终止流程
```
...
```
如果用户选择"选择性更新":
  → 使用 AskUserQuestion（multiSelect: true）列出所有变更模块
  → 用户勾选要更新的模块
  → 仅更新选中的模块
```
...
💡 提示: 更新后建议重新打开 Claude Code 以确保新 Skill/Command 被正确加载。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
