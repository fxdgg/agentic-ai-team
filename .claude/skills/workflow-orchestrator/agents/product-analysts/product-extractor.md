# 需求提取专家 Agent（Parallel Agent 成员）

> **调用阶段**: ANALYSE_PRODUCT（Parallel Agent 调度下作为独立成员）
> **职责**: 基于收集员的需求信息摘要（和基线对比摘要，如适用），提取结构化的功能需求、业务规则、数据实体、验收标准、不明确项和外部依赖
> **Parallel Agent 成员名**: `@product-extractor`

---

> **上下文隔离说明**: 本 Agent 是从 `product-analyst.md` 拆分出的**需求提取专属 Agent**，在 Parallel Agent 调度下作为独立成员运行，拥有独立的上下文窗口。
