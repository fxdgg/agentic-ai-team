...
| `{skill-root}` | Skill 安装路径（自动注入） | `.claude/skills/workflow-orchestrator/` |
...
│   │   ├── analyse-tech-formats.md  #     ANALYSE_TECH Parallel Agent 专用格式
│   │   ├── architect-backend-formats.md # ARCHITECT_BACKEND Parallel Agent 专用格式
│   │   ├── implement-formats.md     #     IMPLEMENT Parallel Agent 专用格式
│   │   └── build-verify-formats.md  #     BUILD_VERIFY Parallel Agent 专用格式
