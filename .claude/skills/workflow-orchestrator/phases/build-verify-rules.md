...
2. 如果 buildVerifyMode 已有值且非空：
   a) "agent-teams" → 进入 §4.7.4 断点恢复策略（Level 1 恢复分支）
...
```
创建一个名为 verify-{需求ID} 的编译验证团队，使用委派模式。

团队任务：对 IMPLEMENT 阶段产出的代码执行编译验证。
团队成员按平台划分，每个成员负责独立平台的构建验证，互不干扰。
...
为每个启用的平台生成一个独立成员。成员的初始 Prompt 必须包含充分的上下文：

**平台 → 成员映射表**:

| 平台 | 成员名 | Agent 文件 | 负责维度 |
|------|--------|-----------|----------|
...
```
生成一个 {平台中文名} 编译验证成员（{成员名}），Prompt 如下：
...
3. 检查已完成的成员产出的编译验证结果，确定 Level 2 的起始平台
...
| 失败场景 | 处理方式 |
|---------|---------|
| 某平台 Task 返回空结果 / 无编译验证产物 | 降级到 Level 3（已完成平台的结果保留） |
...
```json
{
  ...
  "buildVerifyPipeline": {
    ...
    "degradeReason": "成员 @web-build-verifier 超时无响应",
    "completedAt": "{ISO8601时间}"
  }
}
```
...
2. 根据 buildVerifyMode 值选择恢复策略：
   ...
   --- Level 1 恢复（buildVerifyMode = "agent-teams"）---
   a) 检查各平台 report.md 中是否已有编译验证章节
   b) 根据已完成的平台，创建新的 Agent Team 只包含未完成平台的成员
...
**Step 2: 执行** —
- **Level 1 (Parallel Agent)**: 创建验证团队 → 分配任务（全并行） → 监控完成 → 超时检测 → 汇总结果 → 清理团队
