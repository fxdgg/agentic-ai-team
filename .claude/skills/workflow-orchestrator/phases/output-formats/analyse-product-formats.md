# ANALYSE_PRODUCT 阶段 Parallel Agent 输出格式
...
## 13. ANALYSE_PRODUCT 阶段 Parallel Agent 预览格式

```
** 📋 阶段预览: ANALYSE_PRODUCT（Parallel Agent 调度） **

📌 需求: [需求名称]
🔄 当前阶段: ANALYSE_PRODUCT (1/13)
🚀 调度模式: Parallel Agent 四成员串行协作（三级降级）
...
🔄 降级策略:
  - L1: Parallel Agent（当前） → L2: Task 串行管道 → L3: orchestrator 直接执行
...
## 14. ANALYSE_PRODUCT 阶段 Parallel Agent 进度监控格式

```
** 🔄 ANALYSE_PRODUCT 进度: Parallel Agent **
...
## 15. ANALYSE_PRODUCT 阶段 Parallel Agent 总结格式

```
** ✅ 阶段完成: ANALYSE_PRODUCT（Parallel Agent 调度） **

🏷️ 团队: analyse-product-[需求ID]
⏱️ 总耗时: [时长]
🚀 调度模式: Parallel Agent（{N} 个成员串行协作）
📋 需求类型: [全新需求 / 迭代增量需求]
