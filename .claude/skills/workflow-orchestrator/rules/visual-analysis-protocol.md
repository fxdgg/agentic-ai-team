...
2. 检测文件大小：
   ...
   - > 20MB → 标记 oversized = true，必须预处理后再分析（Read 有 20MB 限制）
```
...
> - 文件体积极大（Figma 长页面导出可达 30MB+），超过 Read 限制
...
IF 用户消息包含 Figma URL:
  ...
  2. 检查 MCP 可用性:
     - 读取 .claude/mcp.json → 检查 FramelinkFigmaMCP 配置
     - IF MCP 可用:
...
2. 当用户通过对话框直接粘贴图片时：
   a) 图片以临时路径存在于对话上下文中
   b) 使用 Write 工具将图片内容保存到 design-screenshots/ 目录
...
| **SVG 预处理优先** | SVG 文件**必须先经过 §0.2 预处理流程**转换为 PNG 后再进入分析。直接对 Figma 导出的大型 SVG 做 `Read` 会因文字转路径而严重有损 |
