...
> **核心工具**: Claude Code IDE 内置的 `read_lints` 工具
...
`read_lints` 是 Claude Code IDE 提供的实时诊断工具，底层对接语言服务器（Language Server），能够返回当前工作区中的诊断信息（errors、warnings、hints），**无需手动执行编译命令**。
...
**触发时机**: 每次通过 `Write` 或 `replace_in_file` 修改/创建文件后。
