...
| **发现多个 Git 仓库** | 向用户展示仓库列表和改动概况，使用 `AskUserQuestion` 让用户选择推送目标 |
| **发现 0 个 Git 仓库** | 提示用户"当前工作区未检测到 Git 仓库"，建议执行 `git init` 初始化，然后**停止** |

#### 0-3. 多仓库时的用户选择交互

当检测到多个仓库时，使用 `AskUserQuestion` 工具展示选择器。
...
- 写出其他开发者能理解的简洁摘要。
- 摘要行中**不要**包含文件路径或技术噪音。
- 提交信息中**不要**提及"AI 生成"或"Claude Code"。
...
- `.claude/skills/workflow-orchestrator/` — 工作流编排专家技能
