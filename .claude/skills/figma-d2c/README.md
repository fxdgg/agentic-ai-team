...
将 Figma 设计稿自动转换为可直接运行的前端代码。基于 Claude Code Skill 机制，通过 Figma MCP 获取设计数据，结合项目级配置文件实现**精确还原、稳定复现、可追溯验证**的 Design-to-Code 工作流。
...
本 Skill 依赖 **Framelink Figma MCP** 获取设计稿数据，需要在 Claude Code 中连接：
...
```bash
# 图片批量压缩与 WebP 转换
node .claude/skills/figma-d2c/scripts/process-images.cjs --dir src/assets/images --webp --quality 85

# 视觉回归对比
node .claude/skills/figma-d2c/scripts/vrt-check.cjs --figma-image design.png --local-image screenshot.png

# 资源上传 CDN
export D2C_CDN_DOMAIN="cdn.example.com"
node .claude/skills/figma-d2c/scripts/upload-figma-images.cjs --dir src/assets/images
```

## 目录结构

```text
.claude/skills/figma-d2c/
