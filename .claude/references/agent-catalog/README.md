# Agent Catalog — Claude Code 原生 Agent 注册表
# ====================================
#
...
在 Claude Code IDE 原生架构中，Agent Catalog 是 **参考资料（references）**——
workflow-orchestrator 读取这些定义来了解每个 Agent 的能力和使用方式，
然后通过 Task 工具或 Parallel Agent 成员方式来实际调度。

> ⚠️ 注意：实际的 Agent 执行定义（Prompt 模板、行为约束等）
> 位于 `.claude/skills/workflow-orchestrator/agents/` 目录下的 Markdown 文件中。
...
| 适用于自动化调度匹配 | 适用于 Task 工具 / Parallel Agent 注入 |
| 通用参数化模板 | 具体角色的完整指令 |
...
4. 通过 **Task 工具** 或 **Parallel Agent 成员** 方式调度执行

### 新增 Agent

1. 在对应类别目录下创建 YAML 文件
2. 按照上述规范填写所有必填字段
3. 在 `.claude/skills/workflow-orchestrator/agents/` 下创建对应的 Markdown 执行定义
